...
const S3Uploader = () => {
  ...
  useEffect(() => {
    async function fetchBuckets() {
      try {
        const response = await axios.get('http://dsm-vot-apiaws.duckdns.org/buckets');
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post(`http://dsm-vot-apiaws.duckdns.org/buckets/${selectedBucket}/upload`, formData, {
  ...
  const handleBucketChange = async (bucketName) => {
    ...
    try {
      const response = await axios.get(`http://dsm-vot-apiaws.duckdns.org/buckets/${bucketName}`);
